...
async def _find_place_by_prompt(
    ...
    # 넓은 음식 카테고리 fallback은 특정 음식명 검색 실패를 감추므로 제외
    # (예: "햄버거" 없으면 "음식점" fallback으로 아무 식당이나 선택되는 문제 방지)
    ...
    # fallback 키워드 → 카카오 카테고리 코드 매핑 (더 정확한 검색)
    _kw_to_code = {"음식점": "FD6", "카페": "CE7", "편의점": "CS2", "술집": "FD6"}
    # 카테고리별 추가 제외 필터
    _code_excludes = {
